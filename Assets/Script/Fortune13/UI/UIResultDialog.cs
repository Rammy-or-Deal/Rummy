...
public class UIResultDialog : MonoBehaviour
{
    ...
    internal void SetProperty(UICalcDialog calcDlg)
    {
        ...
        {            
            ...
            GameMgr.Inst.Log("calcPlayer[" + i + "].IsSet=" + m_calc_player[i].IsSeat+", gold="+m_calc_player[i].totalCoin, LogLevel.FortuneLuckyLog);
    ...
    private int AddLuckyBonus(UICalcDialog calcDlg)
    {
        ...
        foreach (var player in pList.m_playerList)
        {
            ...
            GameMgr.Inst.Log("Origianl Coin actor="+player.m_actorNumber+", coin:=" + uiCalcPlayer.totalCoin, LogLevel.FortuneLuckyLog);
            uiCalcPlayer.totalCoin += int.Parse(uiLuckyPlayer.resultText.text);            
            GameMgr.Inst.Log("Penalty from Lucky. actor="+player.m_actorNumber+", penalty:=" + int.Parse(uiLuckyPlayer.resultText.text), LogLevel.FortuneLuckyLog);
            GameMgr.Inst.Log("Result Coin actor="+player.m_actorNumber+", coin:=" + uiCalcPlayer.totalCoin, LogLevel.FortuneLuckyLog);
